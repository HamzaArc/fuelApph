  ...
  return (
    <div className="flex flex-col h-full bg-background-dark animate-fadeIn">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 space-y-4 pb-24 no-scrollbar">
        {MOCK_STATIONS.map((s) => {
          const isBest = s.prices.Diesel === 13.40;
          return (
            <div 
              key={s.id}
              onClick={() => onStationSelect(s)}
              className={`relative group bg-surface-dark rounded-2xl p-5 border transition-all active:scale-[0.98] ${
                isBest ? 'border-primary/40 shadow-xl shadow-primary/5' : 'border-white/5'
              }`}
            >
              {isBest && (
